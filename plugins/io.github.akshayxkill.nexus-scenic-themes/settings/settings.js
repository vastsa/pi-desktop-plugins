(() => {
  ...
  const sync = () => { range.value = String(values.get(active)); output.textContent = `${range.value}px`; cards.querySelectorAll("button").forEach((card) => card.setAttribute("aria-pressed", String(card.dataset.theme === active))); };
  const save = async (id, value, token) => { try { await invoke("themes.setVariables", { themeId: id, values: { "--nexus-backdrop-blur": value } }); const saved = Object.fromEntries(values); await invoke("plugin.setSettings", { scenicBackdropBlurByTheme: saved }); if (token === generation && id === active) { confirmed = value; setStatus(""); } } catch { if (token === generation && id === active) { values.set(id, confirmed); sync(); setStatus("Could not save backdrop blur. The previous value was restored."); } } };
  ...
  const initialize = async () => {
    try {
      const [appearance, settings] = await Promise.all([invoke("app.getAppearance"), invoke("plugin.getSettings")]);
      const persisted = settings?.scenicBackdropBlurByTheme;
      if (persisted && typeof persisted === "object") for (const [id, value] of Object.entries(persisted)) if (values.has(id)) values.set(id, clamp(value));
