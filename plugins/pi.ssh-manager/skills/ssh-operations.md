...
3. Treat `ssh_execute` as a high-risk remote shell action. Explain the command
   before running it when it changes state, and ask before destructive work. The
   plugin conservatively blocks deletion, overwrites, database changes, service
   and permission changes, package mutations, container deletion, shell
   chaining, redirection, and ambiguous dynamic execution by default.
...
6. If a command is blocked, tell the user to review it and check the one-time
   approval box in the SSH Manager panel. Never try to pass an override flag;
   AI authorization is intentionally unavailable.
7. Use `ssh_disconnect` when the remote task is complete. Sessions are logical
