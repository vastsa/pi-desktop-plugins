...
- **账号管理**：阿里云 / 腾讯云 / Cloudflare / AWS Route53 / 火山引擎 / 华为云 凭据统一管理
  - 凭据经 **PBKDF2(150000) + AES-GCM** 加密后仅存本机，主密码只保存在内存中
  - 进入面板时强制设置/解锁主密码；忘记密码可一键清空全部本地数据
...
- 账号凭据：PBKDF2 派生密钥 + AES-GCM 加密，密文存储在面板 localStorage；
- 主密码：仅存于内存，关闭面板即失效，忘记密码只能清空数据；
