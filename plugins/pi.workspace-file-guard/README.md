...
不申请文件系统、网络、剪贴板或后台服务权限。插件本身不写盘、不改 ACL、不联网；真正落盘的仍是 Agent 自己的 Write / Bash。

## 安装

1. PI-Desktop → 扩展 → 安装插件包，选择 `packages/pi.workspace-file-guard-0.2.4.piplug`。
...
- `explicit=true` 只是模型自称用户点名了目标；桌面、下载、文档和系统临时目录即使标了也禁止。
