...
import { localeHref, localeLabel } from "../lib/i18n";
...
export function LanguageSwitcher({ locale, label, availableLocales }: { locale: Locale; label: string; availableLocales: string[] }) {
  ...
  return <select className="language-switcher" aria-label={label} value={locale} onChange={(event) => router.push(localeHref(currentPath, event.target.value))}>
