...
const english: SiteCopy = {
  ...
  docs: {
    home: "Home", kicker: "For builders", title: "Build a plugin.", description: "PI-Desktop plugins are small, versioned packages that add tools, panels, skills and workflows to a local workspace.", quickStart: "Quick start", qualityTitle: "What makes a good plugin?", qualityItems: ["Clear README explaining what the plugin does and what it can access.", "Semantic versioning and a short changelog for every published version.", "A minimum permission set with plain-language safety notes.", "Localized panel titles when a plugin provides a UI panel."], submitTitle: "Publish to the registry", submitDescription: "Sign in at plugins.aiuo.net, create a publisher, then upload with pi-plugin publish. GitHub is only a fallback mirror of the live catalog.", readGuide: "Read CONTRIBUTING.md", localTitle: "Local verification", localDescription: "Load the development plugin in PI-Desktop, confirm its commands and panels, inspect the permission behavior, then test the packaged .piplug before publishing.", templateTitle: "Start from a template", templateDescription: "Copy the practical workspace summary template or the smallest Hello example.", permissionTitle: "Request only what you need", permissionDescription: "Permissions are reviewed by users and should be kept as narrow as possible.", packageTitle: "Pack and publish", packageDescription: "Build a .piplug package and publish it to plugins.aiuo.net. Do not PR catalog.json.", sourceFirst: "The live catalog is plugins.aiuo.net. This GitHub repo mirrors it hourly and keeps official plugin source.",
  },
};
...
const simplifiedChinese: SiteCopy = {
  ...
  docs: { home: "首页", kicker: "面向开发者", title: "开发插件。", description: "PI-Desktop 插件是小型、版本化的扩展包，可以为本地工作区增加工具、面板、技能和工作流。", quickStart: "快速开始", qualityTitle: "什么样的插件更好？", qualityItems: ["README 清楚说明插件做什么，以及会访问什么。", "每个发布版本使用语义化版本号，并附带简短变更记录。", "只申请必要权限，并用易懂的语言说明风险。", "提供 UI 面板时，同时填写本地化的面板标题。"], submitTitle: "发布到插件中心", submitDescription: "在 plugins.aiuo.net 登录并创建 publisher，然后用 pi-plugin publish 上传。GitHub 只是官方 catalog 的回退镜像。", readGuide: "阅读 CONTRIBUTING.md", localTitle: "本地验证", localDescription: "在 PI-Desktop 中加载开发插件，确认命令和面板，检查权限行为，然后在发布前测试打包后的 .piplug。", templateTitle: "从模板开始", templateDescription: "复制实用的工作区摘要模板，或最小化的 Hello 示例。", permissionTitle: "只申请真正需要的权限", permissionDescription: "用户会检查插件权限，权限范围应尽可能小。", packageTitle: "打包并发布", packageDescription: "构建 .piplug 并发布到 plugins.aiuo.net，不要再向 catalog.json 提 PR。", sourceFirst: "线上目录是 plugins.aiuo.net。本 GitHub 仓库每小时镜像一次，并保留官方插件源码。" },
};
